...
using Library.Infrastructure.Data; // Add this for JsonData
...
public class ConsoleApp
{
    ...
    public ConsoleApp(ILoanService loanService, IPatronService patronService, IPatronRepository patronRepository, ILoanRepository loanRepository, JsonData jsonData)
    {
        ...
        _jsonData = jsonData; // Initialize JsonData
    }
    ...
    async Task<ConsoleState> SearchBooks()
    {
        ...
        var activeLoans = _jsonData.Loans?.Where(l => bookItems.Any(bi => bi.Id == l.BookItemId) && l.ReturnDate == null).ToList();

        if (activeLoans == null || activeLoans.Count < bookItems.Count)
        {
            Console.WriteLine($"'{book.Title}' is available for loan.");
        }
        else
        {
            var loan = activeLoans.First();
